...
この関数はRamanujanのモックテータ関数をモジュラー形式論の枠組みで理解するためにZwegersによって導入されたもので, その後\ruby{擬}{モック}モジュラー形式という概念を導入する契機となった対象である. 
...
\begin{rem}
	...
	\end{quote}
	という主張に言い換えられる（ここで「調和Maas形式 (harmonic Maass form of weight $ 2 $)」, 「正則部分 (holomorphic part)」, 「\ruby{擬}{モック}モジュラー形式 (mock modular form of weight $ 2 $)」の定義はそれぞれ\cite[Definition 4.2, Definition 4.4, Definition 5.16 i)]{BFOR}を見よ）. 
...
また松坂俊輝さんには2020年に14回以上にわたる個人的なセミナーにて\ruby{擬}{モック}テータ関数やZwegersの不定値テータ関数, Vign\'{e}rasの結果や関連する研究について懇切丁寧に教えて頂きました. 
